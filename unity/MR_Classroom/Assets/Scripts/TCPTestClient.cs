...
public class TCPTestClient : MonoBehaviour
{
    ...
    Dictionary<GameObject, float> t0 = new Dictionary<GameObject, float>();
    Dictionary<GameObject, float> t1 = new Dictionary<GameObject, float>();
    ...
    public void InterpretMarker(int markerId, int spawnId)
    {
        ...
        else if (grabbableObjects.ContainsKey(markerId))
        {
            if (grabbableObjects[markerId].activeSelf == false && grabbableObjects[markerId].GetComponent<OrganelleController>().locked == false)
            {
